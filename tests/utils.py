...
def copy_structure_db(filepath: str, engine: Engine):
    ...
    with engine.connect() as conn:
        ...
        if len(structures_rows) > 0:
            conn.execute(
                insert(target_table), [row._mapping for row in structures_rows]
            )
        conn.commit()
...
def copy_job_data_db(filepath: str, engine: Engine):
    ...
    with source_as_db.connect() as source_conn:
        ...
        exprun_rows = source_conn.execute(select(tables.experiment_run_table)).all()
    ...
    with engine.connect() as conn:
        ...
        if len(job_data_rows) > 0:
            conn.execute(insert(target_table),[row._mapping for row in job_data_rows])
        # Experiment run
        target_table = tables.table_change_schema(expid, tables.experiment_run_table)
        conn.execute(CreateTable(target_table, if_not_exists=True))
        if len(exprun_rows) > 0:
            conn.execute(insert(target_table),[row._mapping for row in exprun_rows])
        conn.commit()
...
def copy_graph_data_db(filepath: str, engine: Engine):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(CreateTable(target_table, if_not_exists=True))
        if len(graph_rows) > 0:
            conn.execute(insert(target_table),[row._mapping for row in graph_rows])
        conn.commit()
...
def copy_autosubmit_db(filepath: str, engine: Engine):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(insert(tables.ExperimentTable),[row._mapping for row in exp_rows])
        conn.execute(CreateTable(tables.DetailsTable.__table__, if_not_exists=True))
        conn.execute(insert(tables.DetailsTable),[row._mapping for row in details_rows])
        conn.commit()
...
def copy_as_times_db(filepath: str, engine: Engine):
    ...
    # Copy data to the Postgres database
    with engine.connect() as conn:
        conn.execute(CreateTable(tables.ExperimentStatusTable.__table__, if_not_exists=True))
        conn.execute(insert(tables.ExperimentStatusTable),[row._mapping for row in as_times_rows])
        conn.commit()
...
def copy_job_packages_db(filepath: str, engine: Engine):
    ...
    with engine.connect() as conn:
        ...
        if len(job_packages_rows) > 0:
            conn.execute(insert(target_table),[row._mapping for row in job_packages_rows])
        ...
        if len(wrapper_job_packages_rows) > 0:
            conn.execute(insert(target_table),[row._mapping for row in wrapper_job_packages_rows])
        conn.commit()
