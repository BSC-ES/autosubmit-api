...
class ExperimentHistory:
    def __init__(
        ...
        experiment_history_db_manager: ExperimentHistoryDbManager,
        logger: Logging,
    ) -> None:
        self.expid = expid
        self._log = logger
    ...
    def get_historic_job_data(self, job_name: str) -> List[Dict[str, Any]]:
        ...
        for job_data_dc in post_job_data_dcs:
            run_id_to_POST_job_data_dcs.setdefault(job_data_dc.run_id, []).append(
                job_data_dc
            )
            if (
                job_data_dc.run_id,
                job_data_dc.rowtype,
            ) not in run_id_wrapper_code_to_job_data_dcs:
                run_id_wrapper_code_to_job_data_dcs[
                    (job_data_dc.run_id, job_data_dc.rowtype)
                ] = self.manager.get_job_data_dc_COMPLETED_by_wrapper_run_id(
                    job_data_dc.rowtype, job_data_dc.run_id
                )
        ...
        for job_data_dc in all_job_data_dcs:
            if job_data_dc.run_id not in run_id_to_experiment_run_involved:
                run_id_to_experiment_run_involved[job_data_dc.run_id] = (
                    self.manager.get_experiment_run_by_id(job_data_dc.run_id)
                )
            if (
                job_data_dc.run_id,
                job_data_dc.rowtype,
        ...
        for job_data_dc in all_job_data_dcs:
            experiment_run = run_id_to_experiment_run_involved.get(
                job_data_dc.run_id, None
            )
            jobs_in_package = run_id_wrapper_code_to_job_data_dcs.get(
                (job_data_dc.run_id, job_data_dc.rowtype), []
            )
            if experiment_run:
                average_post_time = 0.0
                post_job_data_dcs_in_run = run_id_to_POST_job_data_dcs.get(
                    job_data_dc.run_id, []
                )
                if len(post_job_data_dcs_in_run) > 0:
                    for post_job_data_dc in post_job_data_dcs_in_run:
                        if post_job_data_dc.rowtype > Models.RowType.NORMAL:
                            jobs_in_package_for_post_job = (
                                run_id_wrapper_code_to_job_data_dcs.get(
                                    (post_job_data_dc.run_id, post_job_data_dc.rowtype),
                                    [],
                                )
                            )
                            average_post_time += (
                                post_job_data_dc.queuing_time_considering_package(
                                    jobs_in_package_for_post_job
                                )
                            )
                        else:
                            average_post_time += (
                                post_job_data_dc.queuing_time
                                + post_job_data_dc.running_time
                            )
                    average_post_time = average_post_time / len(
                        post_job_data_dcs_in_run
                    )
            result.append(
                {
                    "counter": job_data_dc.counter,
                    ...
                    "queue_time": job_data_dc.delta_queueing_time_considering_package(
                        jobs_in_package
                    ),
