...
from autosubmit_api.components import jobdata as JobData
...
def get_experiment_data(expid):
    ...
    try:
        ...
        try:
            experiment_status = ExperimentStatusDbRepository().get_status(expid)
        ...
        try:
            result["description"] = ExperimentDbRepository().get_by_expid(expid).get("description", "NA")
        except Exception:
...
def quick_test_run(expid):
    ...
    try:
        status = ExperimentStatusDbRepository().get_status(expid)
        if status != "RUNNING":
            running = False
    except Exception as exp:
        error = True
...
def search_experiment_by_id(
    ...
    """
    result = list()
    query_result, _ = ExperimentJoinDbRepository().search(
...
def get_current_running_exp():
    ...
    """
    result = list()
    query_result, _ = ExperimentJoinDbRepository().search(only_active=True)
