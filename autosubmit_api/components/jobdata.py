...
from autosubmit_api.database.repositories import ExperimentRunDbRepository
...
class JobDataStructure:
    ...
    def __init__(self, expid: str, basic_config: APIBasicConfig):
        ...
        """
        self.db_version = 99 # Previous versions are unsupported

        self.exp_run_db = ExperimentRunDbRepository(expid)
    ...
    def get_max_id_experiment_run(self):
        ...
        """
        try:
            current_experiment_run = self.exp_run_db.get_last_run()

            if current_experiment_run:
                return ExperimentRun(
                    run_id=current_experiment_run.get("run_id"),
                    created=current_experiment_run.get("created"),
                    start=current_experiment_run.get("start"),
                    finish=current_experiment_run.get("finish"),
                    chunk_unit=current_experiment_run.get("chunk_unit"),
                    chunk_size=current_experiment_run.get("chunk_size"),
                    completed=current_experiment_run.get("completed"),
                    total=current_experiment_run.get("total"),
                    failed=current_experiment_run.get("failed"),
                    queuing=current_experiment_run.get("queuing"),
                    running=current_experiment_run.get("running"),
                    submitted=current_experiment_run.get("submitted"),
                    suspended=current_experiment_run.get("suspended", 0),
                    metadata=current_experiment_run.get("metadata", ""),
                    modified=current_experiment_run.get("modified"),
                )
            else:
                return None
    ...
    def get_experiment_run_by_id(self, run_id: int):
        """
        Get experiment run stored in database by run_id
        """
        try:
            current_experiment_run = self.exp_run_db.get_run_by_id(run_id)
            ...
            Log.warning(
                "Autosubmit couldn't retrieve experiment run. get_experiment_run_by_id. Exception {0}".format(str(exc)))
            return None

    def get_current_job_data(self, run_id: int):
        """
        ...
        """
        try:
            current_job_data = self.job_data_db.get_last_job_data_by_run_id(run_id)
                ...
                for job_data in current_job_data:
                    current_collection.append(
                        JobData(
                            ...
                            rowtype=job_data.get("rowtype"),
                            ncpus=job_data.get("ncpus"),
                            wallclock=job_data.get("wallclock"),
                            qos=job_data.get("qos"),
                            energy=job_data.get("energy"),
                            date=job_data.get("date"),
                            section=job_data.get("section"),
                            member=job_data.get("member"),
                            chunk=job_data.get("chunk"),
                            last=job_data.get("last"),
                            platform=job_data.get("platform"),
                            job_id=job_data.get("job_id"),
                            extra_data=job_data.get("extra_data"),
                            nnodes=job_data.get("nnodes"),
                            run_id=job_data.get("run_id"),
                            MaxRSS=job_data.get("MaxRSS", 0),
                            AveRSS=job_data.get("AveRSS", 0),
                            out=job_data.get("out", ""),
                            err=job_data.get("err", ""),
                            rowstatus=job_data.get("rowstatus", 0),
                        )
                    )
